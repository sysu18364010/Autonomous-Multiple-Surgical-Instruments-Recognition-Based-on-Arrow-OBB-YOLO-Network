...
def test(cfg,
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # print("inf+train")
            # print(inf_out[0][0])
            # print(train_out[0][0,0,0,0])
            # Compute loss
            if is_training:  # if model has loss hyperparameters
                # print('yes?')
                loss += compute_loss(train_out, targets, model)[1][:7]  # GIoU, obj, cls,arrows
        ...
        for si, pred in enumerate(output):
            ...
            # Clip boxes to image bounds， 他是通过这样子来保证包围盒的坐标是在图片里面的，
