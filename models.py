...
def create_modules(module_defs, img_size, cfg):
    ...
    # 把第一个net的参数pop掉，那还要读取他作甚？
    _ = module_defs.pop(0)  # cfg training hyperparams (unused)

    # 三通道图片？
    output_filters = [3]  # input channels
    module_list = nn.ModuleList()
    # routs是啥，需要再来了解了解，现在知道了，YOLO输出层的前一层
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            layers = mdef['layers']
            # 每一层的输出时append到output_filters里面的，所以-1直接取最后一个，从头数的话，因为最开始时是3通道【这个代表输入】，所以要+1
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers, stride):
        ...
        self.no = nc + 9  # number of outputs (85) 
        # 初始化一些变量ng，有几个网格吗？
    ...
    def forward(self, p, out):
        ...
        # 在这里重构了张量的维度，这里是没有进行转换的，需要看一下loss funtion怎么处理的
        # print("yolo:",p.shape)
        # p = p.view(bs, self.na, self.no, self.ny, self.nx).permute(0, 1, 3, 4, 2).contiguous()  # prediction
        # 添加四个坐标,首先将48-3*16，再交换一下位置
        p = p.view(bs, self.na, self.no, self.ny, self.nx).permute(0, 1, 3, 4, 2).contiguous()  # prediction
        # print("resized",p.shape)
        ...
        if self.training:
            ...
            # print(self.create_grids((nx, ny), p.device))
            io[..., :2] = torch.sigmoid(io[..., :2]) + self.grid  # xy
            io[..., 2:4] = torch.exp(io[..., 2:4]) * self.anchor_wh  # wh yolo method
            io[..., :4] *= self.stride# 前面这些都乘了网格大小
            torch.sigmoid_(io[..., 4:])# 将最后几个映射到0，1之间，这个定义和之前相同可以，接下来看如何训练传播误差
            # print("io,p:",io.shape,p.shape)
